...
```{r}
r <- raster("2009_5.nc", varname = "sst4")
```
...
xy <-  data.frame(X = coords$lon, Y = coords$lat)
...
temps <- raster::extract(r, res)
```

### Joit exposure to master DB

```{r}
coords <- cbind(coords, temps)
